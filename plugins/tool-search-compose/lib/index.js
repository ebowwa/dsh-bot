 ...
 * the budgets the shipped grep/glob tools run on, not private ones. Names and
 * shapes verified against the real @deepseek-ai/dsh-tool-fs-search
 * (runRipgrep's 7-parameter signature, the four named constants, and the
 * return shape carrying `noMatches` — confirmed by both PR #43 reviews on a
 * dsh lane carrying dsh 0.1.0-rc.7).
 */
...
export function apply(ctx) {
  const tool = defineTool({
    ...
    parameters: {
      ...
      context: { type: "integer", description: "context lines on both sides of each match (--context; incompatible with countOnly/filesOnly)" },
